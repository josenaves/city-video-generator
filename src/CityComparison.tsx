...
const CityStats: React.FC<{
    ...
    const parseData = (val: number) => {
        ...
        if (isCurrency) {
            ...
            // Default suffix handling if provided
            if (prop.includes('idhm')) {
                decimals = 3;
            } else if (prop.includes('Score')) {
                decimals = 0;
            }
        }
    ...
    return (
        <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center' }}>
            <h1 style={{ fontSize: 80, marginBottom: 50, fontFamily: 'sans-serif', color: 'white', textShadow: '0 4px 10px rgba(0,0,0,0.5)' }}>{label}</h1>
            <div style={{ display: 'flex', gap: 50, flexDirection: 'row', alignItems: 'flex-start' }}>
...
export const CityComparison: React.FC = () => {
    ...
    // Calculate score dynamically based on wins in the comparisons list
    let wins1 = 0;
    let wins2 = 0;
